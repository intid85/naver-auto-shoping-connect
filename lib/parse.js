...
function parseFolder(folder) {
  ...
  // 같은 폴더의 참고글.txt에 저장된 기존 쇼핑커넥트 발급 정보를 읽는다.
  let connect = null;
  const refPath = path.join(folder, "참고글.txt");
  if (fs.existsSync(refPath)) {
    const ref = fs.readFileSync(refPath, "utf8");
    const issuedUrl = ref.match(/발급링크\s*:\s*(https?:\/\/\S+)/)?.[1] || null;
    const shortUrl = ref.match(/단축링크\s*:\s*(https?:\/\/\S+)/)?.[1] || null;
    const productId = ref.match(/쇼핑커넥트\s*:\s*id\s*(\d+)/i)?.[1] || null;
    const productName = ref.match(/^제품\s*:\s*(.+)$/m)?.[1]?.trim() || null;
    if (issuedUrl || shortUrl || productId) connect = { issuedUrl, shortUrl, productId, productName };
  }
